...
class SwaggerJAVA
{
     ...
     * @return array
     */
    public function getDatastructureList($base_url, $module_url, $param, $repository_type, $private_token, $owner)
    {
        $api_structure_list = array();
        $api_model_content = array();
        $model_name_content = array();
        $tool = new \tool\module\Tool();
        $tool->getContentForSwagger($base_url, $module_url, $param, $api_model_content, $repository_type, $private_token, $owner, 1);
        foreach($api_model_content as $model_content)
        {
            $result = $tool->getUrlContentForSwagger($model_content, 10, $repository_type, $private_token, $owner);
            $result = json_decode($result, TRUE);
            $model_content = base64_decode($result['content']);
            // 除去import部分
     ...
     * @return array
     */
    public function getApiData($api_structure_list, $base_url, $api_dir_path, $param, $repository_type, $private_token, $owner)
    {
        ...
        $api_content_list = array();
        $tool = new \tool\module\Tool();
        $tool->getContentForSwagger($base_url, $api_dir_path, $param, $api_content_list, $repository_type, $private_token, $owner, 1);
        foreach($api_content_list as $content)
        {
            $result = $tool->getUrlContentForSwagger($content, 10, $repository_type, $private_token, $owner);
            $result = json_decode($result, TRUE);
