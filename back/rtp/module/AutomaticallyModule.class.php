...
class AutomaticallyModule
{
	...
	public static function start()
	{
		...
		if (FIRST_DEPLOYMENT)
			FileModule::createSecurityIndex();

		//判断是否有PATH_INFO信息，如果没有则无需路由
		if (!isset($_SERVER['PATH_INFO']))
			throw new ExceptionModule(11001, 'url in lack of pathinfo for ' . $_SERVER["REQUEST_URI"]);
		...
		if (!isset(self::$groupName))
			throw new ExceptionModule(11002, 'error in lack of groupName for ' . $_SERVER["REQUEST_URI"]);
		elseif (!isset(self::$controllerName))
			throw new ExceptionModule(11003, 'error in lack of controllerName for ' . $_SERVER["REQUEST_URI"]);
		elseif (!isset(self::$operationName))
			throw new ExceptionModule(11004, 'error in lack of operationName for ' . $_SERVER["REQUEST_URI"]);

		//实例化控制器对象
		$class = new \ReflectionClass(self::$groupName . '\controller\\' . self::$controllerName);
		//系统魔术方法
		...
		if (!in_array(strtolower(self::$operationName), $php_magic_methods) && $class -> hasMethod(self::$operationName))
		{
			...
			{
				//操作无法访问
				throw new ExceptionModule(11005, 'operation isn\'t a public function for ' . $_SERVER["REQUEST_URI"]);
			}
		}
		else
		{
			//操作无法访问
			throw new ExceptionModule(11006, 'undefined operation or illegal operation name for ' . $_SERVER["REQUEST_URI"]);
		}
	}
	...
	{
		$path = realpath('./server' . DIRECTORY_SEPARATOR . self::$groupName . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . self::$controllerName . '.controller.php');
		quick_require($path);
	...
	{
		$path = realpath('./server' . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $className) . '.module.php');
		quick_require($path);
	}
	...
	{
		$path = realpath('./server' . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $className) . '.dao.php');
		quick_require($path);
	}
	...
	{
		$path = realpath('./server' . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $className) . '.model.php');
		quick_require($path);
	}
