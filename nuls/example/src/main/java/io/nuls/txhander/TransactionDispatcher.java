...
public final class TransactionDispatcher extends BaseCmd {
    ...
    @Autowired
    UserAliasSetProcessor userAliasSetProcessor;

    @Autowired
    UserUpdateImageProcessor userUpdateImageProcessor;

    @Autowired
    UserUpdateInfoProcessor userUpdateInfoProcessor;

    @Autowired
    ProjectUpdateProcessor projectUpdateProcessor;

    @Autowired
    ProjectCreateProcessor projectCreateProcessor;

    @Autowired
    ProjectSupportProcessor projectSupportProcessor;

    @Autowired
    ProjectDeleteProcessor projectDeleteProcessor;
    ...
    private TransactionProcessor getProcessor(int txType) {
        ...
        if (txType == userAliasSetProcessor.getType()) {
            return userAliasSetProcessor;
        }
        if (txType == userUpdateImageProcessor.getType()) {
            return userUpdateImageProcessor;
        }
        if (txType == userUpdateInfoProcessor.getType()) {
            return userUpdateInfoProcessor;
        }
        if (txType == projectSupportProcessor.getType()) {
            return projectSupportProcessor;
        }
        if (txType == projectUpdateProcessor.getType()) {
            return projectUpdateProcessor;
        }
        if (txType == projectCreateProcessor.getType()) {
            return projectCreateProcessor;
        }
        if (txType == projectDeleteProcessor.getType()) {
            return projectDeleteProcessor;
        }
        if (txType == transferProcessor.getType())
        {
